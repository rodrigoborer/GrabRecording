...
def step_cdr():
    ...
    try:
        ...
        start_time = f"{today}T00:00-03:00"
        ...
        def cdr_fields(r):
            return r.get("main_cdr") or r

        with_rec = [r for r in all_records if (cdr_fields(r).get("recordfiles") or "").strip()]

        sorted_with_rec = sorted(
            with_rec,
            key=lambda r: cdr_fields(r).get("start", "") or "",
            reverse=True,
        )

        recording = None
        for r in sorted_with_rec:
            f = cdr_fields(r)
            rec_file = (f.get("recordfiles") or "").strip()
            if rec_file:
                recording = {
                    "cdrId":       f.get("AcctId", ""),
                    "caller":      f.get("src", ""),
        ...
        if len(all_records) == 0:
            return err(
                f"Nenhuma chamada encontrada hoje ({today}). "
                "Verifique se ha chamadas registradas no PBX para hoje.",
        ...
        if not recording:
            return err(
                f"Foram encontradas {len(all_records)} chamadas hoje, mas nenhuma possui gravacao. "
